...
#ifndef HMATRIX_SSE_H_
#define HMATRIX_SSE_H_
...
#if PRECISION == 1
  HMatrix(const __m128 entries[4]);

  inline void matMult(const __m128 mat[4], __m128 resMat[4]=NULL) const;
  inline void matMultIP(const __m128 mat[4]);
  inline void matVecMult(const HVector& vec, __m128& vec_entries) const;
         bool invertMatrix(FLOATING const mat[16], FLOATING inv[16]) const;
         void switchRow(FLOATING mat[4][8], int row1, int row2) const;
#else
  HMatrix(const __m128d entries[8]);

  inline void matMult(const __m128d mat[8], __m128d resMat[8]=NULL) const;
  inline void matMultIP(const __m128d mat[8]);
  inline void matVecMult(const HVector& vec, __m128d vec_entries[2]) const;
         bool invertMatrix(FLOATING const mat[16], FLOATING inv[16]) const;
         void switchRow(FLOATING mat[4][8], int row1, int row2) const;
#endif //PRECISION == 1
...
#if PRECISION == 1
#include "matrix_sse_sp.inl"
#else
#include "matrix_sse_dp.inl"
#endif //PRECISION == 1

#endif //HMATRIX_SSE_H_